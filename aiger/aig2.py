...
def _eval(circ, inputs, latches=None):
    ...
    lookup = fn.merge(
        ...
        {l.input: latches.get(name, l.initial) for name, l in circ.lmap.items()}
    )
...
class AIG2:
    ...
    def cutlatches(self, latches):
        new_names = {name: _fresh() for name in latches}
        ...
        for name in latches:
            ...
            imap[new_name] = latch.input
            omap[new_name] = latch.id

        circ = attr.evolve(self, 
            imap=imap.persistent(),
            omap=omap.persistent(),
            lmap=lmap.persistent()
        )
        return circ, {n: (k, self.lmap[n].initial) for n, k in new_names.items()}
    ...
    def unroll(self, horizon, *, init=True, omit_latches=True,
        ...
        if init:
            src = {timed_name(n, 0): init for n, init in l_map.values()}
            unrolled = source(src) >> unrolled
